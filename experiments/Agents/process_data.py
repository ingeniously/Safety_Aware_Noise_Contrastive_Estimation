...
sys.path.append("/home/choi/Safety_Aware_Noise_Contrastive_Estimation/trajectron")
...
nl = 0
l = 0
maybe_makedirs('/home/choi/Safety_Aware_Noise_Contrastive_Estimation/experiments/processed')
data_columns = pd.MultiIndex.from_product([['position', 'velocity', 'acceleration'], ['x', 'y']])
for desired_source in ['eth', 'hotel', 'univ', 'zara1', 'zara2']:
    for data_class in ['train', 'val', 'test']:
        ...
        scenes = []
        data_dict_path = os.path.join('/home/choi/Safety_Aware_Noise_Contrastive_Estimation/experiments/processed', '_'.join([desired_source, data_class]) + '.pkl')
        ...
        for subdir, dirs, files in os.walk(os.path.join('raw', desired_source, data_class)):
            for file in files:
                if file.endswith('.txt'):
                    ...
                    data['frame_id'] -= data['frame_id'].min()   # Start from 0
