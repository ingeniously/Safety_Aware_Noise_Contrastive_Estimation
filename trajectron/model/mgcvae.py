...
class MultimodalGenerativeCVAE(object):
    ...
    def obtain_encoded_tensors(self,
        ...
        :param labels: Label tensor including the label output for each agent over time [bs, t, pred_state].        Note: not used
        ...
        self.dynamic.set_initial_condition(initial_dynamics)                # Note: provide global info
    ...
    def p_y_xz(self, mode, x, x_nr_t, y_r, n_s_t0, z_stacked, prediction_horizon,
        ...
        # TODO: h_state should be regularized, it is the core representation of x and z
        # (1) extract 
        # (2) return
        ...
        for j in range(ph):
            ...
            if mode != ModeKeys.PREDICT:
                h_state_stack.append(
                    h_state.reshape(
                        num_samples, num_components, -1, initial_state.shape[-1]
                    ).permute(0, 2, 1, 3)
                )
        ...
        if mode == ModeKeys.PREDICT:
            ...
            h_state_stack = torch.stack(h_state_stack, dim=1)
            return y_dist, h_state_stack
    ...
    def decoder(self, mode, x, x_nr_t, y, y_r, n_s_t0, z, labels, prediction_horizon, num_samples):
        ...
        :param x: Input / Condition tensor.
        :param x_nr_t: Joint state of node and robot (if robot is in scene).
        :param y: Future tensor.                                                                    # Note: not used
        :param y_r: Encoded future tensor.
        :param n_s_t0: Standardized current state of the node.
        ...
        y_dist, h_stack = self.p_y_xz(mode, x, x_nr_t, y_r, n_s_t0, z,
    ...
    def train_loss(self,
        ...
        # Remove colliding data
        x = x[safety_gt]
        y_e = y_e[safety_gt]
        y = y[safety_gt]
        n_s_t0 = n_s_t0[safety_gt]
        labels = labels[safety_gt]

        inputs = inputs[safety_gt]
        neighbors_prev = neighbors_prev[safety_gt]
        primary_next = primary_next[safety_gt]
        neighbors_next = neighbors_next[safety_gt]
        ...
        log_p_y_xz, h_stack = self.decoder(mode, x, x_nr_t, y, y_r, n_s_t0, z,
        ...
        # # Revert colliding data
        # log_likelihood = (torch.sum(log_p_y_xz_mean[safety_gt]) - torch.sum(log_p_y_xz_mean[~safety_gt].clamp(min=-1000.0))) / log_p_y_xz_mean.shape[0]
        ...
        if self.hyperparams['contrastive_weight'] > 0:
            loss_nce = self.snce.loss(inputs, neighbors_prev, primary_next, neighbors_next, h_stack.permute(1, 2, 0, 3))
