...
def main():
    ...
    if args.load_dir and os.path.exists(args.load_dir):
        ...
        with open(os.path.join(args.load_dir, 'config.json'), 'r') as config_json:
            hyperparams = json.load(config_json)
    else:
        if not os.path.exists(args.conf):
            raise RuntimeError('Config json not found!')

        print(f"Load hyperparams config from {args.conf}")
    ...
    trajectron.set_environment(train_env, snce)
    ...
    for node_type in train_env.NodeType:
        ...
        if args.pretrain:
            print('>>> Pretrain SSL')
            param_optim = [{'params': head_projection.parameters()},
                            {'params': encoder_sample.parameters()}]
        else:
            print('>>> Joint train')
            if args.contrastive_weight > 0:
                param_optim = [{'params': model_registrar.get_all_but_name_match('map_encoder').parameters()},
                ...
                param_optim = [{'params': model_registrar.get_all_but_name_match('map_encoder').parameters()},
                        {'params': model_registrar.get_name_match('map_encoder').parameters(), 'lr':0.0008}]
        optimizer[node_type] = optim.Adam(param_optim, lr=hyperparams['learning_rate'])
